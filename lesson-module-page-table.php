<?php
    ...
    $query_view_table = "SELECT * FROM lessonModulesPages";
    $query_run_view_table = mysqli_query($con, $query_view_table);

    // Used to display the drop down menu of list of lesson modules when adding/editing a module page:
...
<html>
    <head>
        ...
        <title>TASK Lesson Modules</title>
    ...
    <body style="background-color: white;" id="page-top">
             ...
             <div class = "position-relative">
                 <div class="position-absolute top-0 start-50 translate-middle-x">
                     <div class = "card shadow rounded text-bg-dark mb-3 text-center m-3" style="max-width:200rem; min-width: 75rem;">
                        <h1 class= "mt-4" >TASK Module Page Information</h1>
                        <hr>
                        <p class = "p-4">This page will be responsible for the maintenece of detailing page information PER module. The admin can filter pages based on which module is selected.
                            Admins will also have the ability to upload an image and videos within a module page. 
                        </p>
                        </p>
                    ...
                    <div class = "container text-center">
                        <button type="button" class="btn btn-success m-1" data-bs-toggle="modal" data-bs-target="#addScreenModal">Add A Page from Selected Module</button>
                        <a href="lesson-modules-table.php"><button type="button" class="btn btn-warning m-1">Manage Lesson Modules</button></a>
                    </div>
                    ...
                    <div class = "mt-5 table-responsive" style="max-width:100rem;" id = "link_wrapper">
                        ...
                        <table id="modules_pages_table" class = "table stripe">
                            <thead>
                                <tr>
                                    <th scope="col">Module Page ID</th>
                                    <th scope="col">Module ID</th>
                                    <th scope="col">Module Page Header</th>
                                    <th scope="col">Module Info Text</th>
                                    <th scope="col">Module Info Image</th>
                                    <th scope="col">Module Info Video</th>
                            ...
                            <tbody>
                                ...
                                <?php
                                    ...
                                    {
                                ?>
                                    <td><?php echo $row['module_page_id']; ?></td>
                                    <td><?php echo $row['module_id']; ?></td>
                                    <td style="max-width: 20rem; "><div style="white-space: nowrap; overflow-x: hidden;"><?php echo $row['module_page_header']; ?></div></td>
                                    <td style="max-width: 20rem; "><div style="white-space: nowrap; overflow-x: hidden;"><?php echo $row['module_info_text']; ?></div></td>
                                    <td><?php echo $row['module_info_image']; ?></td>
                                    <td><?php echo $row['module_info_video']; ?></td>
                                    <td><?php echo $row['date_created']; ?></td>
                                    <td>
                                        <a href="table_operations/lesson-module-page-table-view.php?id=<?= $row['module_page_id']; ?>" class="btn btn-sm"><i class="fa-solid fa-eye"></i></a>
                                        <a href="table_operations/lesson-module-page-table-edit.php?id=<?= $row['module_page_id']; ?>" class="btn btn-sm"><i class="fa-solid fa-pen-to-square"></i></a>

                                        <!-- Make sure to pass ID to modal deletion screen -->
                                        <button data-id="<?= $row['module_id']; ?>" type="button" data-bs-toggle="modal" name="set_id_btn" data-bs-target="#confirmationDeletionModal"><i class="fa-solid fa-trash" style="color: red;"></i></button>
                                    </td>
                                </tr>
                                <?php
                                    }
            ...
            <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable">
                <div class="modal-content">
                    <div class="modal-header">
                        <h1 class="modal-title fs-5" id="exampleModalLabel">Add New Module Page</h1>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">

                        <form action="php/lesson_module_page_db_operations.php" method="POST" enctype="multipart/form-data" >
                            <p><strong>NOTE:</strong> Please make sure to select which module you want to add this page in first!</p>
                            <hr>
                            <div class = "mb-3">
                                ...
                                <div class="dropdown">
                                    ...
                                    <select class="form-select" name="module-choice">
                                        ...
                                        <?php 
                                            ...
                                            {
                                        ?>
                                                <option value="<?php echo $row['module_id']?>" ><?php echo $row['module_id']?> - <?php echo $row['module_name']?></option>
                                        <?php 
                                            }
                                        ?>
                                    </select>
                            ...
                            <div class="mb-3">
                                <label for="module_page_header" class="form-label">Module Page Header</label>
                                <input type="text" name="module_page_header" id="module_page_header" class="form-control" placeholder="Statistics of Food Insecurity" required>
                            </div>
                            <div class="mb-3">
                                <label for="module_info_text" class="form-label">Module Info Text</label>
                                <textarea class="form-control" name="module_info_text" id="module_info_text" rows="7" required></textarea>
                                <!-- <div class="form-text">Lesson Module Summary</div> -->
                            </div>
                            <div class="mb-3">
                                <label for="module_info_image" class="form-label">Upload Module Page Image</label>
                                <input type="file" name="module_info_image" id="module_info_image">
                                <div class="form-text">Lesson Module Page Image (Only Accepts: 'jpg','png','jpeg','gif')</div>
                            </div>
                            <div class ="mb-3">
                            ...
                            <div class="mb-3">
                                <label for="module_info_video" class="form-label">Upload Module Page Video</label>
                                <input type="file" name="module_info_video" id="module_info_video">
                                <div class="form-text">Lesson Module Page Video (Only Accepts: 'mp4','avi','mov','mpeg')</div>
                            </div>
                            <!-- Need to fix later on -->
                            <div class ="mb-3">
                                <label class="form-label">Video Preview</label>
                                <img id="videoPreview" src = "#" style="max-width: 200px;"/>
                            </div>

                            <div class="modal-footer">
                                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
                                <!-- Execute SQL query when button is pressed -->
                                <a href="."><button type="submit" name = "add_module_page" class="btn btn-primary">Add Module Page</button></a>
                            </div>
                        </form>
                    </div>
                </div>
        ...
        <div class="modal fade" id="confirmationDeletionModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <p>
                            Are you sure you want to delete this entry? Deletion of a module 
                            will also delete the associated quiz and quiz question(s). 
                    ...
                    <form action= "php/lesson_module_page_db_operations.php" method ="POST" class="d-inline" enctype="multipart/form-data">
                        <div class ="p-3">
                            <input type ="hidden" name="module_id" id="module_id" value=""  class="form-control"  ></input>
                            <input type="text" id="set_id_btn" name="delete_module" class="form-control" required>
                        </div>
                    </form>
                </div>
            </div>
        ...
        <script>
            module_info_image.onchange = evt => {
                const [file] = module_info_image.files
                if(file){
        ...
        <script>
            $('#confirmationDeletionModal').on('show.bs.modal', function(event){
                ...
                modal.find('#module_page_id').val(getID);
