<?php
    ...
    $query_view_table = "SELECT * FROM users";
...
<html>
    <head>
        ...
        <title>TASK Users (Students) </title>
             ...
             <div class = "position-relative">
                 <div class="position-absolute top-0 start-50 translate-middle-x">
                     <div class = "card shadow rounded text-bg-dark mb-3 text-center m-3" style="max-width:200rem; min-width: 75rem;">
                        <h1 class= "mt-4" >TASK User (Student) Management</h1>
                        <hr>
                        <p class = "p-4">This page will be responsible for the maintenece of the current users (Students) who are currently using the web application. The following table will allow you to add, view, delete, and edit user information.
                        </p>
                        <p class ="p-1">
                            NOTE: Sensitive information such as passwords are not presented here- <strong style="color: red;" >only First/Last name of the student and their email is presented.</strong> Email is required from the user to reset their passwords
                        </p>
                    ...
                    <div class = "container text-center">
                        <button type="button" class="btn btn-success m-1" data-bs-toggle="modal" data-bs-target="#addScreenModal">Add New User (Student)</button>
                    </div>
                    ...
                    <div class = "mt-5 table-responsive" style="max-width:100rem;" id = "link_wrapper">
                        ...
                        <table id="users_table" class = "table stripe">
                            <thead>
                                <tr>
                                    ...
                                    <th scope="col"># of Completed Modules</th>
                                    <th scope="col">Date Created</th>
                                    <th scope="col">Last Login (PWA) </th>
                            ...
                            <tbody>
                                ...
                                ?>
                                    ...
                                    <td><?php echo $row['number_completed_modules']; ?></td>
                                    <td><?php echo $row['date_created']; ?></td>
                                    <td><?php echo $row['last_login']; ?></td>
                                    <td>
                                        <a href="table_operations/users-view.php?id=<?= $row['user_id']; ?>" class="btn btn-sm"><i class="fa-solid fa-eye"></i></a>
                                        <a href="table_operations/users-edit.php?id=<?= $row['user_id']; ?>" class="btn btn-sm"><i class="fa-solid fa-pen-to-square"></i></a>

                                        <!-- Make sure to pass ID to modal deletion screen -->
                                        <button data-id="<?= $row['user_id']; ?>" type="button" data-bs-toggle="modal" name="set_id_btn" data-bs-target="#confirmationDeletionModal"><i class="fa-solid fa-trash" style="color: red;"></i></button>                                          
            ...
            <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable">
                <div class="modal-content">
                    <div class="modal-header">
                        <h1 class="modal-title fs-5" id="exampleModalLabel">Add New User (Student) </h1>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">

                        <form action="php/users_db_operations.php" method="POST" enctype="multipart/form-data">
                            <div class="mb-3">
                                <label for="user_email" class="form-label">User (Student) Email</label>
                                <input type="email" name="user_email" id="user_email" class="form-control" placeholder="abc@123.com" required>
                            </div>
                            <div class="mb-3">
                                <label for="user_first_name" class="form-label">First Name</label>
                                <input type="text" name="user_first_name" id="user_first_name" class="form-control" placeholder="Jane" required>
                            </div>
                            <div class="mb-3">
                                <label for="user_last_name" class="form-label">Last Name</label>
                                <input type="text" name="user_last_name" id="user_last_name" class="form-control" placeholder="Doe" required>
                            </div>

                            <!-- Call Password confirmation script goes here before adding -->
                            <div class="mb-3">
                                <label for="user_password" class="form-label">User (Student) Password</label>
                            ...
                            <div class="mb-3">
                                <label for="re-user_password" class="form-label">Re-enter User Password</label>
                                <input type="password" name="re-user_password" id="re-user_password" class="form-control" autocomplete="off" placeholder="Enter Password" required>
                            </div>

                            <div class="modal-footer">
                                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
                                <!-- Execute SQL query when button is pressed -->
                                <a href="."><button type="submit" name = "add_user" class="btn btn-primary">Add User</button></a>
                            </div>
                        </form>
                    </div>
                </div>
        ...
        <div class="modal fade" id="confirmationDeletionModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <p>
                            Are you sure you want to delete this user? Deletion of a student will require the student to re-register through the mobile application. <strong>All student progress will be lost.</strong>
                           
                        </p>
                        <p>Please type <strong style="color: red;" >DELETE</strong> (case sensitive) to confirm this action.</p>
                    </div>

                    <!-- Process form and check if DELETE keyword is passed through before removal of row from database -->
                    <form action= "php/users_db_operations.php" method ="POST" class="d-inline" enctype="multipart/form-data">
                        <div class ="p-3">
                            <input type ="hidden" name="user_id" id="user_id" value=""  class="form-control"  ></input>
                            <input type="text" id="set_id_btn" name="delete_user" class="form-control" required>
                        </div>
                    </form>
                </div>
            </div>
